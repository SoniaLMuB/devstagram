        ...
        <div class="md:w-1/2 p-10 bg-white rounded-lg shadow-xl mt-10 md:mr-0">
            <form action="{{route('post.store')}}" method="POST" novalidate>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="descripcion" id="descripcion" placeholder="Descripción " class="borde p-3 w-full rouded-lg @error('descripcion') border-red-500 @enderror">{{old('descripcion')}}</textarea>
                    @error('descripcion')
                        <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">{{$message}} </p>
                    @enderror
                </div>
                ...
                <input type="submit" value="Publicar" class="bg-sky-600 hove:bg-sky-700 transition-colors cursor-pointer uppercase font-bold w-full p-3 text-white rounded-lg">
            </form>
